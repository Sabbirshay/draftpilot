...
\`\`\``;
  ...
  test('extractSenderName: filters out blacklist terms and redacted PII', () => {
    ...
    assert.strictEqual(extractSenderName('Dear concern,\nI want to join you as a partner.'), 'there');
    assert.strictEqual(extractSenderName('Dear sir/madam,\nRegarding my inquiry.'), 'there');
    assert.strictEqual(extractSenderName('Dear Madam/Sir,\nRegarding my inquiry.'), 'there');
    assert.strictEqual(extractSenderName('Dear sir / madam,\nRegarding my inquiry.'), 'there');
    assert.strictEqual(extractSenderName("Dear Ma'am,\nRegarding my inquiry."), 'there');
    assert.strictEqual(extractSenderName('Dear Whom It May Concern,\nInquiry.'), 'there');
    assert.strictEqual(extractSenderName('Dear User,\nNotice.'), 'there');
    assert.strictEqual(extractSenderName('Dear Client,\nNotice.'), 'there');
    assert.strictEqual(extractSenderName('Hello customer,\nHere is your receipt.'), 'there');
    assert.strictEqual(extractSenderName('From: [EMAIL_REDACTED]\nSubject: Hello'), 'there');
    assert.strictEqual(extractSenderName('Hi, Alice\nHow are you?'), 'Alice');
    assert.strictEqual(extractSenderName('Dear Mr. Anderson,\nRegarding contract.'), 'Anderson');
  });

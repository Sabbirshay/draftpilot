...
export default function AdminGuard({ children }: { children: React.ReactNode }) {
  ...
  // 1. AUTHORIZED & UNLOCKED: Render Superadmin Dashboard immediately (supports passkey-only root sessions)
  if (isAdminUnlocked) {
    return <>{children}</>;
  }

  // 2. UNLOGGED VISITOR: Show Secure Admin Gateway Login (Master Passkey or Admin Credentials)
  if (!user) {
    return (
  ...
  // 3. LOGGED IN AS NON-ADMIN: Show 403 Forbidden Access Screen (with passkey override)
  if (!isEmailAdmin) {
    return (
      <div className="min-h-screen bg-bg text-text flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-lg bg-elevated/90 border border-red-500/30 rounded-3xl p-8 shadow-2xl backdrop-blur-xl text-center">
          ...
          {authError && (
            <div className="mt-4 p-3 rounded-xl bg-red-500/15 border border-red-500/30 text-red-300 text-xs font-mono text-left">
              ⚠️ {authError}
            </div>
          )}

          {/* Master Passkey Direct Override */}
          <form onSubmit={handleUnlockWithPasskey} className="mt-6 p-4 rounded-2xl bg-bg/80 border border-border text-left space-y-3">
            <div>
              <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-1 font-mono flex items-center justify-between">
                <span>Unlock with Master Passkey</span>
                <span className="text-[10px] text-accent lowercase">direct root access</span>
              </label>
              <input
                type="password"
                value={passkeyInput}
                ...
                className="w-full px-4 py-2 rounded-xl bg-elevated border border-border text-sm text-text placeholder:text-text-dim focus:outline-none focus:border-accent font-mono"
              />
            </div>
            <button
              type="submit"
            ...
            >
              Verify Master Passkey &amp; Enter Console →
            </button>
          </form>
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/dashboard"
              className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-elevated border border-border hover:border-accent/40 text-text font-bold text-xs transition-all shadow-sm"
            >
  ...
  // 4. LOGGED IN AS ADMIN BUT NEEDS PASSKEY CONFIRMATION: Show Quick Passkey Unlock
  return (
    <div className="min-h-screen bg-bg text-text flex items-center justify-center p-4 relative overflow-hidden">
